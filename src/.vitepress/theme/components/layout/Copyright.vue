...
const data = reactive({
  isEnable: theme.value.copyrightConfig.isEnable ?? true,
...
const { isEnable, isOriginal, author, authorLink, articleTitle, articleLink } =
  toRefs(data);
</script>

<template>
  <div v-if="isEnable" class="copyright">
    <div class="content">
      <div class="item">
        ...
        <span>
          <a
            ...
            >{{ author }}</a
          >
          <a v-else :href="authorLink" title="进入作者主页" target="_blank">{{
            author
          }}</a>
        </span>
      </div>
